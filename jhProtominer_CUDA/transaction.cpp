...
{
	if( numberOfTxHashes > 2097152 )
	{
		// we only support 32 transactions at a time
		printf("\ncalculateMerkleRoot: Too many transactions, numberOfTx set to 2097152\n");
		numberOfTxHashes = 2097152;
	}
	if(numberOfTxHashes <= 0 )
	{
		printf("\ncalculateMerkleRoot: Block has zero transactions (not even coinbase)\n");
	...
	else
	{
		...
		for(uint32 f=0; f<10; f++)
		{
			if( layerSize[f] == 0 )
			{
				printf("\ncalculateMerkleRoot: Error generating merkleRoot hash\n");
				free(hashData);
				return;
			}
